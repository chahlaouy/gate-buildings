<x-app-layout>
    ...
    <div class="bg-white rounded-2xl p-4" style="box-shadow: 0 4px 6px -1px rgb(0 0 0 / 10%), 0 2px 4px -1px rgb(0 0 0 / 6%);">
        ...
        <form action="{{route('partners.update', $record->id)}}" method="POST" enctype="multipart/form-data">
            @csrf
            <div  x-data="{ result : '', title: '{{$record->name}}' }">
                <div class="mb-4">
                    <x-input
                        x-model="title"
                ...
                <div class="mb-4">
                    <x-input
                        id="siteUrl" class="block mt-1 w-full" type="text" name="siteUrl" :value="$record->siteUrl" autofocus placeholder="{{__('Site Url')}}" />
                </div>
                <div class="mb-4">
                    <textarea id="description" class="block mt-1 w-full rounded-md border-gray-300 focus:border-yellow-200 focus:ring focus:ring-yellow-400 focus:ring-opacity-25"
                            name="description"
                            placeholder="{{__('Description')}}" rows="5">{{$record->description}}</textarea>
                </div>
