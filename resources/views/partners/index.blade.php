<x-app-layout>
    ...
    <div class="bg-white rounded-2xl p-4" style="
            ...
            <tbody class="flex-1 sm:flex-none text-gray-600 tracking-wide leading-loose">
                @forelse ($records as $record)
                    <tr class="flex flex-col flex-no wrap sm:table-row mb-2 sm:mb-0 border-b border-gray-100">
                        ...
                        <td class="py-2">
                            <img src="{{asset('uploads/partners-logo')}}/{{$record->logo}}" alt="" class="w-8 h-8">
                        </td>
                        <td class="py-2 flex items-center">
                            <form action="{{route('partners.destroy', $record->id)}}" method="POST">
                                @csrf
                                @method("DELETE")
                            ...
                            <a href="{{route('partners.edit', $record->id)}}" class="px-2">
                ...
                @empty
                    <tr class="flex flex-col flex-no wrap sm:table-row mb-2 sm:mb-0 border-b border-gray-100">
                        <td colspan="5">
                            No Records<a href="{{route('partners.create')}}" class="text-green-800 underline"> Create One</a>
                        </td>
                    </tr>
                @endforelse
